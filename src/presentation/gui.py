...
class SegmentacionApp:
    ...
    def inicializar(self) -> None:
        """Punto de entrada publico compatible con el diagrama de clases."""
        return None

    def construir_paneles(self) -> None:
        self._build_panels()

    def mostrar_modo(self, modo: str) -> None:
        self._show_mode(modo)
    ...
    def seleccionar_imagen_dataset(self, indice: int) -> None:
        self._set_dataset_index(indice)
    ...
    def ejecutar_en_segundo_plano(self) -> None:
        self._worker_loop()

    def actualizar_imagen(self) -> None:
        self._update_image()

    def capturar_pantalla(self) -> None:
        self._capture_screenshot()

    def cerrar_aplicacion(self) -> None:
        self._on_close()

    def flujo_principal_segmentacion(self) -> Any:
        return segmentacion.algoritmos_segmentacion(mode=self.mode, dataset_index=self.dataset_index)
    ...
    def _build_logo(self, container: tk.Frame) -> None:
        ...
        def _load_logo(filename: str, target_w: int, target_h: int) -> Optional[ImageTk.PhotoImage]:
            img_path = os.path.join(ASSETS_IMAGES_DIR, filename)
            try:

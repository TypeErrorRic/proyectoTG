...
_GROUND_HELPER_SPEC = importlib.util.spec_from_file_location(
    "src.utilities.helpers_ground_detection_for_wall",
    _GROUND_HELPER_PATH,
)
if _GROUND_HELPER_SPEC is None or _GROUND_HELPER_SPEC.loader is None:
    raise ImportError(f"No se pudo cargar helper de camino transitable: {_GROUND_HELPER_PATH}")
ground_utils = importlib.util.module_from_spec(_GROUND_HELPER_SPEC)
_GROUND_HELPER_SPEC.loader.exec_module(ground_utils)
ransac_plane_gpu = ground_utils.ransac_plane_gpu
_refine_plane = ground_utils._refine_plane
...
class Muro:
    ...
    def detectar(self, *args: Any, **kwargs: Any) -> Any:
        resultado = get_wall_planes(*args, **kwargs)

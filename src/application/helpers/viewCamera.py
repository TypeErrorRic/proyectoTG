...
def get_depth_scale(pipeline: rs.pipeline = None) -> float:
    ...
    # Fallback: intenta vÃ­a contexto global
    try:
        ctx = rs.context()
    ...
    # Ãšltimo recurso (comÃºn en D435/D415): 1 mm
...
def extract_depth_raw(frames: rs.composite_frame) -> np.ndarray:
    """
    Devuelve profundidad nativa (uint16, HÃ—W) o None si falta.
    """
...
def extract_depth_meters(frames: rs.composite_frame, depth_scale: float = None, pipeline: rs.pipeline = None) -> np.ndarray:
    """
    Profundidad en metros (float32, HÃ—W). Promedia 10 frames en GPU con CuPy.
    """
    ...
    # Promediar valores vÃ¡lidos en GPU con CuPy
...
def compute_rays_from_intrinsics(intr) -> np.ndarray:
    """
    Calcula rayos por pÃ­xel en coords de cÃ¡mara. Retorna (H, W, 3) con [x/z, y/z, 1].
    """
...
def compute_normalized_rays(H: int, W: int) -> np.ndarray:
    """
    Construye rayos \"normalizados\" para una imagen de tamaÃ±o (H, W)
    cuando no se conocen intrÃ­nsecas reales de cÃ¡mara.

    Devuelve un array (H, W, 3) con vectores [x, y, 1] donde x,y estÃ¡n
...
###############################################
# AlineaciÃ³n DEPTHâ†’COLOR (GPU opcional)
###############################################

# ParÃ¡metros de rendimiento bÃ¡sicos:
...
class DepthToColorAlignerGPU:
    """
        Alineador DEPTHâ†’COLOR en GPU (CuPy + CUDA): precalcula A = R_cd @ ray_d y
        proyecta cada pÃ­xel de depth al plano color con z-buffer atÃ³mico.
    """
    ...
    def align(self, depth_m: np.ndarray) -> np.ndarray:
        """
        Depth alineado al espacio COLOR (HcÃ—Wc) en metros (float32).
        """
        cp = self._cp
        if depth_m is None:
            return None
        # Ajustar tamaÃ±o a (Hd, Wd) si es necesario
        ...
        # Convertir a float32; 0.0 donde quede +inf (sin asignaciÃ³n)
...
def make_depth_to_color_aligner(pipeline: rs.pipeline) -> Callable[[rs.composite_frame], Optional[np.ndarray]]:
    """
    Devuelve una funciÃ³n align(frames)-> depth (float32, HcÃ—Wc) en espacio COLOR.
...
def precompute_rays_for_stream(pipeline: rs.pipeline, stream: rs.stream = rs.stream.depth) -> Tuple[np.ndarray, int, int, Optional[Callable[[rs.composite_frame], Optional[np.ndarray]]]]:
    """
        Precalcula rayos (H, W, 3) para el stream dado. Si stream=color, tambiÃ©n
        retorna una funciÃ³n para alinear depthâ†’color.
    """
    ...
    if stream == rs.stream.color:
        # Preparar funciÃ³n de alineaciÃ³n depthâ†’color
        align_fn = make_depth_to_color_aligner(pipeline)

    return rays_np, H, W, align_fn

# =========================================================
# ==========  I N I C I A L I Z A C I Ã“ N  C Ã M A R A  ===
# =========================================================
def init_camera(
    color_width: int = 640,
    ...
    """
        ...
        Devuelve: (pipeline, params dict para visualizaciÃ³n de nube de puntos).
    """
    pipeline = rs.pipeline()
    config = rs.config()

    # Streams (ajusta resoluciÃ³n segÃºn modelo)
...
# =========================================================
# ============  M A I N   D E   V I S U A L I Z A C I Ã“ N
# =========================================================
    ...
    try:
        ...
        while True:
            ...
            if points_xyz is not None and points_xyz.size > 0:
                colors = rgb_sub[valid]  # IndexaciÃ³n directa con mÃ¡scara 2D
            else:
                colors = None
            ...
            cv2.putText(pc_img, "Controles: A/D yaw  W/S pitch  Q/E roll  Z/X FOV  +/- tamaÃ±o  ESC",
            ...
            if frame_count % 60 == 0:
                ...
                print(f"  ExtracciÃ³n RGB:     {(t2a-t2)*1000:6.2f} ms")
                print(f"  AlineaciÃ³n D->C:    {(t3-t2a)*1000:6.2f} ms")
                print(f"  Resize depth:       {(t4-t3)*1000:6.2f} ms")
                print(f"  Rayos -> PointCloud:{(t5-t4)*1000:6.2f} ms")
                print(f"  ExtracciÃ³n colores: {(t6-t5)*1000:6.2f} ms")

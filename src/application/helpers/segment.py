...
_DATA_DIR = _SRC_ROOT / "infrastructure" / "datasets"
...
def _segmentar_impl(frame_started_at: Optional[float] = None) -> Any:
    ...
    ground_mask = _runtime.camino_transitable.detectar(
        mapaProfundidad,
        rays_cp,
        H,
        W,
    ...
    with _runtime_lock:
        _runtime["last_ransac_ms"] = _runtime.camino_transitable.obtener_tiempo_ransac_ms()
    ...
    wall_res = {}
    try:
        wall_res = _runtime.muro.detectar(
            mapaProfundidad,
            rays_cp,
            H,
            W,
    ...
    try:
        door_mask = _runtime.puerta.detectar(
            imagenRGB,
            ...
            rays=rays_cp,
            ground_normal=_runtime.camino_transitable.last_n_cp,
    ...
    if ground_params.get("ground_mask_refine") and ground_mask is not None and np.any(ground_mask):
        try:
            ground_mask = mascaras.mejorar_mascara_suelo(
                ...
                depth_m=mapaProfundidad,
                rays=rays_cp,
                plane_n=_runtime.camino_transitable.last_n_cp,
                plane_d=_runtime.camino_transitable.last_d_cp,
...
def _preprocesar_impl(
    ...
    src/infrastructure/datasets/{images,depths} instead of the RealSense camera.
...
def _algoritmos_segmentacion_impl(
    ...
    mode:
        ...
        - "prueba": use RGB and depth frames from src/infrastructure/datasets.
...
class Segmentacion:
    ...
    def __init__(self) -> None:
        self.puerta = Puerta()
        self.camino_transitable = CaminoTransitable()
        self.muro = Muro()
        self.camara = camara
        self.dataset_frames = dataset_frames
    ...
    def construir_parametros_muro(
        ...
        if self.camino_transitable.last_n_cp is not None:
            wall_params["ground_normal"] = self.camino_transitable.last_n_cp
        return wall_params

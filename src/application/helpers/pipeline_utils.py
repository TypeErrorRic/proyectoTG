...
def load_dataset_frame(index: Optional[int] = None) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    """
    Load an RGB + depth pair from src/infrastructure/datasets/{images,depths}.
    ...
    try:
        ...
        if depth_raw.dtype == np.uint16:
            ...
            # NYU dataset format: normalized depth [0, 255] â†’ keep as-is

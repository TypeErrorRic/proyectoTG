...
# Variable para activar la impresiÃ³n de tiempos de cada etapa en get_ground
DEBUG_TIMING = False
...
def ransac_plane_gpu(points,
    ...
    while remaining > 0:
        ...
        #    Troceado para reducir memoria en GPU pequeÃ±as
...
def get_ground(
    ...
    if int(cp.sum(valid)) >= 3:
        ...
        # Sesgar hacia los puntos mÃ¡s bajos (altura)
        ...
        # Opcional: limitar el nÃºmero de puntos usados por RANSAC
        ...
        if int(Psub.shape[0]) >= int(min_inliers):
            ...
            if debug_ransac:
                ...
                if _debug_ransac_counter % debug_ransac_every == 0:
                    ...
                    print(f"[ransac] Promedio Ãºltimas {debug_ransac_every} ejecuciones: {avg_ms:.2f} ms")
